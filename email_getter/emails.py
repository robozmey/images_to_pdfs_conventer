...
list = list(dict.fromkeys([x[0] + ' ' + x[1] + ' ' + x[2] + ' ' + x[3] for x in sorted(list(filter(lambda x: fios.count(x[1] + ' ' + x[2] + ' ' + x[3]) > 0, list2)), key = lambda x: (x[1], x[2], x[3]))]))
...
list = list(dict.fromkeys([x[0] for x in sorted(list(filter(lambda x: fios.count(x[1] + ' ' + x[2] + ' ' + x[3]) > 0, list2)), key = lambda x: (x[1], x[2], x[3]))]))
